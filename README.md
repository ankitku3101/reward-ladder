...
public/r/                 built registry — generated, but committed
```

`public/r/` is generated by `npm run registry:build`, and it is **committed
on purpose**: serving it as a plain static file means the install URL works
even if the build step misbehaves on the host. Re-run that script and commit
the result whenever anything in `components/reward-ladder/` changes.
...
Zero configuration and no environment variables — Vercel auto-detects
Next.js, and `public/r/` ships as committed static files.
...
After the first deploy, confirm the registry is live before sending the
install command to anyone:

```bash
curl -I https://<your-deployment>/r/reward-ladder.json   # expect 200
```
